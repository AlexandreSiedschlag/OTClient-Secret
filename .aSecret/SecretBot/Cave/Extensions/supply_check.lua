...
local rawRound = 0
local time = now
SecretBot.CaveBotData =
  SecretBot.CaveBotData or
  {
    refills = 0,
    rounds = 0,
    time = {},
...
local function setCaveBotData(hunting)
  if hunting then
    ...
    supplyRetries = 0
    table.insert(SecretBot.CaveBotData.refillTime, os.difftime(os.time() - SecretBot.CaveBotData.lastRefill))
    SecretBot.CaveBotData.lastRefill = os.time()
    SecretBot.CaveBotData.refills = SecretBot.CaveBotData.refills + 1
  end

  table.insert(SecretBot.CaveBotData.time, rawRound)
  SecretBot.CaveBotData.rounds = SecretBot.CaveBotData.rounds + 1
  missedChecks = 0
end
