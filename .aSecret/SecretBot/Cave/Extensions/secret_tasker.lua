...
if not storage.caveVariables then
...
local checkTaskName = function(data)
    if data then
        for name, _ in pairs(storage.caveVariables.taskInfo) do
            if data == name then
                return true
            end
        end
...
local setupAutoLoot = function(taskName)
    say("!autoloot clear")
    local monsterData = storage.caveVariables.taskInfo[taskName]
...
CaveBot.Extensions.SecretTasker.setup = function()
    CaveBot.registerAction("tasker", "yellow", function(data, retries)
        if dataIsCorrect(data) then
            ...
            if whatToDo == 'setup' then
                ...
                local staminaPotionUse = list[3] and list[3]:trim() or false
                storage.caveVariables.taskName = taskName
                storage.caveVariables.taskInProgress = true
                storage.caveVariables.monsterAmountMax = storage.caveVariables.taskInfo[taskName]['monsters_to_kill']
                storage.caveVariables.staminaPotionUse = staminaPotionUse
                storage.caveVariables.autoLootEnabled = true
                if storage.caveVariables.autoLootEnabled then
                    setupAutoLoot(taskName)
                end
                return true

            elseif whatToDo == 'getTask' then
                CaveBot.Conversation("hi", "tasks", taskName, "yes")
            ...
            elseif whatToDo == "reportTask" then
                CaveBot.Conversation("hi", "report", taskName, "yes")
                resetTaskData()
                return true
            ...
            elseif whatToDo == "checkStamina" then
                ...
                local stamina = tonumber(string.format("%02d", math.floor(stamina() / 60)))
                g_game.talk("Stamina: " .. stamina .. " de " .. storage.caveVariables.staminaMinimum)
                if stamina >= storage.caveVariables.staminaMinimum then
                    CaveBot.gotoLabel(labelTrue)
                else
                    CaveBot.gotoLabel(labelFalse)
                end
    ...
    CaveBot.Editor.registerAction("tasker", "Tasker", {
            value=[["whatToDo, taskName, staminaPotionUse, labelIfTrue, labelIfFalse"

Exemplo:
"setup, Hydras, true"
"checkTask, Hydras, true, HUNT START, REPORT TASK"
"checkStamina, Hydras, true, GO TRAIN, CONTINUE HUNT"
            ]],
            title="SecretTasker",
            multiline = true
        })
...
onTextMessage(function(mode, text)
    print(mode, text)
    local taskMatch = regexMatch(text, storage.caveVariables.monsterRegex)
    if #taskMatch == 1 and #taskMatch[1] == 4 then
        local currentAmount = tonumber(taskMatch[1][2])
        local maxAmount = tonumber(taskMatch[1][3])
        local monsterTaskName = taskMatch[1][4]

        storage.caveVariables.monsterActual = monsterTaskName
        storage.caveVariables.monsterAmountNow = currentAmount
        storage.caveVariables.monsterAmountMax = maxAmount

        -- Salva progresso específico da task
        if storage.caveVariables.taskInfo[monsterTaskName] then
            storage.caveVariables.taskInfo[monsterTaskName].monsters_killed = currentAmount
            storage.caveVariables.taskInfo[monsterTaskName].monsters_to_kill = maxAmount
            storage.caveVariables.taskInfo[monsterTaskName].in_progress = true
            storage.caveVariables.taskInfo[monsterTaskName].done = false
        end
        print("[CaveBot][SecretTasker]: Current amount: " .. currentAmount .. "/" .. maxAmount)
        return
    end
end)
--------------------------------------------------------------
