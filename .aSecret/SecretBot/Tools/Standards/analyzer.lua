...
SecretBot.CaveBotData = SecretBot.CaveBotData or {
  refills = 0,
  rounds = 0,
  time = {},
  lastRefill = os.time(),
...
--button
analyzerButton = modules.client_topmenu.addRightGameToggleButton('botAnalyzersButton', 'SecretBot Analyzers', '/images/topbuttons/analyzers', toggle, false, 999999)
analyzerButton:setOn(false)
  ...
  for k,v in pairs(trackedLoot) do
    ...
    -- only amount have changed, ignore
      ...
      if trackedLoot[tostring(id)] then
        warn("SecretBot[Drop Tracker]: Item already added!")
        name:setText("Set Item to start track.")
        widget:setItemId(0)
        return
      end
    ...
    for k,v in pairs(usedItems) do
      for i=1,#parents do
        local amount = i == 1 and v.count or
                       i == 2 and v.count/(SecretBot.CaveBotData.rounds + 1) or
                       i == 3 and v.count/(SecretBot.CaveBotData.refills + 1)
...
macro(500, function()
    ...
    --stats
    totalRounds:setText(SecretBot.CaveBotData.rounds)
    avRoundTime:setText(niceTimeFormat(avgTable(SecretBot.CaveBotData.time),true))
    totalRefills:setText(SecretBot.CaveBotData.refills)
    avRefillTime:setText(niceTimeFormat(avgTable(SecretBot.CaveBotData.refillTime),true))
    lastRefill:setText(niceTimeFormat(os.difftime(os.time()-SecretBot.CaveBotData.lastRefill),true))
