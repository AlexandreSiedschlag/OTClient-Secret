...
SecretBot = {} -- global namespace for bot variables
SecretBot.BotServerMembers = {}
SecretBot.standTime = now
SecretBot.isUsingPotion = false
SecretBot.isUsing = false
SecretBot.customCooldowns = {}

function logInfo(text)
    local timestamp = os.date("%H:%M:%S")
    text = tostring(text)
    local start = timestamp.." [SecretBot]: "
...
function standTime()
    return now - SecretBot.standTime
end
...
-- changes state of SecretBot variable, can be used in other scripts
...
onTalk(function(name, level, mode, text, channelId, pos)
    ...
    if text == "Aaaah..." then
        SecretBot.isUsingPotion = true
        schedule(950, function() SecretBot.isUsingPotion = false end)
    end
end)
...
if onSpellCooldown and onGroupSpellCooldown then
    onSpellCooldown(function(iconId, duration)
        schedule(1, function()
            if not SecretBot.customCooldowns[lastPhrase] then
                SecretBot.customCooldowns[lastPhrase] = {id = iconId}
            end
        end)
    end)
    ...
    onGroupSpellCooldown(function(iconId, duration)
        schedule(2, function()
            if SecretBot.customCooldowns[lastPhrase] then
                SecretBot.customCooldowns[lastPhrase] = {id = SecretBot.customCooldowns[lastPhrase].id, group = {[iconId] = duration}}
            end
        end)
    end)
else
...
function getSpellData(spell)
    ...
    if not t then
        for k, v in pairs(SecretBot.customCooldowns) do
            if k == spell then
...
macro(100, function()
    SecretBot.isUsing = now < isUsingTime and true or false
end)
...
function isFriend(c)
    ...
    if table.find(storage.playerList.friendList, name) then
        ...
        return true
    elseif SecretBot.BotServerMembers[name] ~= nil then
        CachedFriends[c] = true
        return true
