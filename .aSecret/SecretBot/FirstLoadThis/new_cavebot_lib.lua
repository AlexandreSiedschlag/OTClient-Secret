...
local function CaveBotConfigParse()
    local name = storage["_configs"]["SecretBot/Target/Configs"]["selected"]
    if not name then
        return warn("[SecretBot] Please create a new TargetBot config and reset bot")
    end
    local file = configDir .. "/SecretBot/Target/Configs/" .. name .. ".json"
