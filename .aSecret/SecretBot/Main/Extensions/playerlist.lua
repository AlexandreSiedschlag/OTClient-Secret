        ...
        if config.outfits then
          ...
          if isFriend(spec:getName()) then
            ...
            if storage.BOTserver.outfit then
              local voc = SecretBot.BotServerMembers[spec:getName()]
              specOutfit.addons = 3
              if voc == 1 then
                specOutfit.type = 131
              elseif voc == 2 then
...
local checkStatus = function(creature)
    ...
    if isFriend(specName) then
      ...
      if config.outfits then
        ...
        specOutfit.feet = 88
        if storage.BOTserver.outfit then
          local voc = SecretBot.BotServerMembers[creature:getName()]
          specOutfit.addons = 3
          if voc == 1 then
            specOutfit.type = 131
          elseif voc == 2 then
...
if rootWidget then
    ...
    UI.Button("PLAYER LIST", function()
        ListWindow:show()
        ListWindow:raise()
        ListWindow:focus()
    end)
    ...
    for v = 1, 3 do
        ...
        addButton.onClick = function()
            ...
            if #names == 0 then
              warn("SecretBot[PlayerList]: Name is missing!")
              return
            end
            ...
            for i=1,#names do
              ...
              if name:len() == 0 then
                  ...
                  if not table.find(playerList, name) then
                      ...
                      warn("SecretBot[PlayerList]: Player ".. name .." is already added!")
                      nameTab:setText("")
                  end
                  clearCachedPlayers()
                  refreshStatus()
