...
if true then
  ...
  onTextMessage(function(mode, text)
    ...
    if #re ~= 0 then
        ...
        local voc
        if text:lower():find("sorcerer") or text:lower():find("supreme") then
            voc = "MS"
        elseif text:lower():find("druid") or text:lower():find("diviny") then
            voc = "ED"
        elseif text:lower():find("knight") then
            voc = "EK"
            if text:lower():find("gladiator") then
                voc = "GK"
            end
        elseif text:lower():find("paladin") then
            voc = "RP"
        elseif text:lower():find("hunter") then
            voc = "RH"
        end
